...
local absorb = function(pos)
    ...
    for k=-3,3 do
        ...
        n = core.get_node(p)
        if core.get_item_group(n.name, "lava") ~= 0 then
            core.add_node(p, {name="air"})
            change = true
        end
    end
    end
...
core.register_node("lava_sponge:lava_sponge", {
	...
	groups = {handy=1, hoey=1, oddly_breakable_by_hand = 3, building_block=1},
	on_place = function(itemstack, player, pointed_thing)
		...
		local on_lava = false
		if core.get_item_group(core.get_node(pos).name, "lava") ~= 0 then
			on_lava = true
		end
		local lava_found = core.find_node_near(pos, 1, "group:lava")
		if lava_found then
			on_lava = true
		end
		if on_lava then
			...
			if absorbed then
				core.item_place_node(ItemStack(wet_sponge), player, pointed_thing)
				if awards then
...
core.register_node("lava_sponge:lava_sponge_wet", {
